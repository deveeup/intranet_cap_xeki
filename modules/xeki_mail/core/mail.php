...
class mail
{
    ...
    public function send_email($to,$subject,$html,$array_info=array())
    {
        ...
        $subject=utf8_encode(to_tildes($subject));
        if(strlen($html)==0){
            d("error empty html");
            die();
        }
        ...
        if(!empty($array_info['files_mailgun'])){
            $info_email['attachment']=$array_info['files_mailgun'];
            $array_info['files_mailgun']='';
        }
    ...
    private function send_by_mail_gun($info){
        ...
        try{
            ...
            if(isset($info['attachment'])){
                $aditional_options["attachment"]=$info['attachment'];
            }
            
            $return = $mg->sendMessage($domain, $info,$aditional_options);
        }catch (Exception $e) {
