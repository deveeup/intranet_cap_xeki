...
class config
{
    ...
    */
    public function __construct($config)
    {
        ...
        $this->sql = \xeki\module_manager::import_module("db-sql");


        $this->type=$config['type'];

        // load of cookies 
        if(isset($_SESSION['xeki_CONFIG_LANG'])){
            if($_SESSION['xeki_CONFIG_LANG']!=''){
                $this->lang=$_SESSION['xeki_CONFIG_LANG'];
            }
        }
    ...
    function process_values($values,$lang=false){
        ...
        $variables = $this->get_value_param('box_items');
        $values = $xeki_admin_module->process_data($variables,$values);
    ...
    function get_active_lang(){
        return $_SESSION['xeki_CONFIG_LANG'];
    }
    function set_active_lang($lang){
        // check is exist
        
        if($_SESSION['xeki_CONFIG_LANG']==$lang){
            return true;
        }
        else{
            $lang_res = $this->get_lang($lang);
            if($lang===false ||$lang_res['main']=='on'){
                $_SESSION['xeki_CONFIG_LANG']= $this->get_lang_main();
                $_SESSION['xeki_CONFIG_LANG']="main";
                $this->lang="main";
            }
            else{
                $_SESSION['xeki_CONFIG_LANG']=$lang;
                $this->lang=$lang;
            }
        }
    }
