...
//d("yes im running");
 ...
 \xeki\routes::action('xeki_auth::login', function(){
    ...
    $message_xeki_popup = $xeki_auth->get_value_param("msg_no_valid_user");
    $xeki_popup->add_msg( $message_xeki_popup );
...
\xeki\routes::action('xeki_auth::register', function(){
    ...
    if (count($res) == 0) {
        ...
        $message_xeki_popup = $xeki_auth->get_value_param("msg_new_user");
        $xeki_popup->add_msg( $message_xeki_popup );
    } else
     {

        $xeki_popup->add_msg("Ya existe una cuenta asociada a este correo.");
        $message_xeki_popup = $xeki_auth->get_value_param("msg_user_exist");
        $xeki_popup->add_msg( $message_xeki_popup );
...
\xeki\routes::action('xeki_auth::update_edit_pass', function(){
    ...
    $res = $sql->update($xeki_auth->getTableUser(), $data, 'id="' . $USER["id"] . '"');
    //    ag_pushMsg('Tu contraseña ha sido actualizada');
    $xeki_popup->add_msg("Tu contraseña ha sido actualizada.");
});
...
\xeki\routes::action('xeki_auth::recover_pass_update', function(){
    ...
    if($response){
        ...
        $xeki_popup->add_msg("El codigo de actualizacion ya no es valido, intenta recuperar de nuevo.");
    }
});
...
\xeki\routes::action('xeki_auth::recover_pass', function(){
    ...
    //    ag_pushMsg('Tu contraseña ha sido actualizada');
    $xeki_popup->add_msg("Se ha enviado a tu correo las instrucciones.");
});
