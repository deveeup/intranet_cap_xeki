...
//            $_CODE
        ...
        $mail->send_email($email,"Confirm account",$content,$extra_data);
        return true;
    ...
    {
        // get user by code
        $_CODE = hash($this->encryption_method, $password);


        $res = $this->sql->query("SELECT {$this->field_user} FROM {$this->table_user} WHERE {$this->field_recover_code}='$code_recover'");
        if (!$res) {
            return false;
        }
        $user = $res[0];
...
//            d("logged");
    ...
    function checkLogin()
    {
        ...
        else \xeki\core::redirect($this->login_page . '?from=' . $this->logged_page);
    }
     ...
     * @return bool
     */
    function pageLoginCheck($level = 1) ## for register or login page
    {
        $this->check_data();
        $this->logged_page;
